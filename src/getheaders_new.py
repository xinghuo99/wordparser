...
class HeaderExtractor:
    ...
    def __init__(self, doc_or_path, extract_field_codes: bool = False):
        """
        初始化页眉提取器
        
        :param doc_or_path: Word 文档路径（字符串）或 Document 对象
        :param extract_field_codes: 是否提取域代码（如 DOCPROPERTY、MERGEFORMAT 等）。
                                   默认 False，只提取用户可见的文本；设为 True 可提取完整的域代码。
        """
        ...
        self._extract_field_codes = extract_field_codes
    ...
    def _deep_extract_text(self, element, nsmap=None):
        ...
        for child in element:
            ...
            if tag == 't':
                ...
                # instrText 包含域代码（如 DOCPROPERTY、MERGEFORMAT 等）
                # 默认不提取，因为这些是 Word 内部代码，用户在文档中看不到
                if self._extract_field_codes and child.text:
                    parts.append(child.text)
            elif tag == 'delText':
                if child.text:
                    parts.append(child.text)
...
if __name__ == '__main__':
    # 方式1：使用文档路径初始化（默认不提取域代码）
    extractor = HeaderExtractor('复杂页眉.docx')
    
    # 方式2：提取域代码（包含 DOCPROPERTY、MERGEFORMAT 等）
    # extractor = HeaderExtractor('复杂页眉.docx', extract_field_codes=True)
    
    # 方式3：使用 Document 对象初始化
